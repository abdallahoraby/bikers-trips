...
// Endpoint to get all Finished trips

function listFinishedTrips(){
    ...
    if( count($user_trips) > 0 ){
        ...
        for( $i=0; $i < count($user_trips); $i++ ){
            ...
            $trips_data['user_trips'][$i]['trip_author_id'] = $user_trips[$i]->post_author;
...
add_action( 'rest_api_init', function (){
    ...
    register_rest_route( API_PATH, '/listFinishedTrips' , array(
        'methods' => 'GET', // define the method GET or POST
        'callback' => 'listFinishedTrips', // define the function that we call to get or post these data
    ) );
