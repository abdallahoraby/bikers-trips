...
// Endpoint to get all live trips

function listLiveTrips(){
...
//            $trips_data['user_trips'][$i]['trip_posted_since'] = human_time_diff($posted_date,current_time( 'U' ));
            ...
            $track_table = $wpdb->prefix.'bikers_tracking';
            $track_query = $wpdb->get_results ( "SELECT * FROM $track_table WHERE `trip_id` = $trip_id  ");
            $trips_data['user_trips'][$i]['trip_tracking_info'] = $track_query;
...
add_action( 'rest_api_init', function (){
    ...
    register_rest_route( API_PATH, '/listLiveTrips' , array(
        'methods' => 'GET', // define the method GET or POST
        'callback' => 'listLiveTrips', // define the function that we call to get or post these data
    ) );
